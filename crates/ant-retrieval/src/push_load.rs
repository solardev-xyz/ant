...
// ---------------------------------------------------------------------------
// Per-peer area-of-responsibility estimates (perf-lab Experiment 5)
// ---------------------------------------------------------------------------

/// What pushes through a peer have taught us about it (hoverfly's
/// 3-bucket storage-radius routing). A receipt whose recovered storer
/// overlay IS the dialed peer proves that peer stores; a valid receipt
/// signed by someone else proves the peer forwarded. Either way the
/// receipt carries the storer's advertised `storage_radius`, which for
/// the self-stored case bounds the peer's area of responsibility.
#[derive(Default, Clone, Copy)]
struct PeerAor {
    /// Latest self-stored receipt's storage radius (`None` until the
    /// peer itself has signed a receipt for us).
    own_radius: Option<u32>,
    storer_hits: u32,
    forwarder_hits: u32,
}

/// Candidate class for the dispatcher's bucket sort, front to back.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub enum AorClass {
    /// Confirmed storer whose radius covers this chunk's proximity.
    InAorStorer = 0,
    /// No receipt-derived observation yet — fresh peers rank ahead of
    /// known forwarders (hoverfly: valid only because dead/slow/at-cap
    /// peers are pre-filtered before classification).
    Unknown = 1,
    /// Observed, but never as an in-AOR storer for this range.
    Forwarder = 2,
}

/// Process-wide receipt-derived AOR book, sharing the swarm state like
/// [`PushLoadTracker`]. Enabled via `ANT_PUSH_AOR_SORT=1`.
#[derive(Default)]
pub struct AorBook {
    peers: Mutex<HashMap<PeerId, PeerAor>>,
}

impl AorBook {
    #[must_use]
    pub fn from_env() -> Option<Self> {
        let v = std::env::var("ANT_PUSH_AOR_SORT").ok()?;
        let v = v.trim();
        (!v.is_empty() && v != "0" && !v.eq_ignore_ascii_case("false")).then(Self::default)
    }

    /// Record a verified receipt obtained by dialing `peer`.
    /// `peer_overlay` is the dialed peer's overlay from the live set;
    /// `storer_overlay`/`storage_radius` come from the receipt.
    pub fn record_receipt(
        ...
        let mut peers = self.peers.lock().expect("aor mutex");
        let e = peers.entry(peer).or_default();
        if storer_overlay == peer_overlay {
            e.storer_hits += 1;
            e.own_radius = Some(storage_radius);
        } else {
            e.forwarder_hits += 1;
        }
    }

    /// Classify `peer` as a push candidate for a chunk at proximity
    /// `chunk_po` to the peer.
    #[must_use]
    pub fn classify(&self, peer: &PeerId, chunk_po: u8) -> AorClass {
        let peers = self.peers.lock().expect("aor mutex");
        match peers.get(peer) {
            None => AorClass::Unknown,
            Some(a) => match a.own_radius {
                Some(r) if u32::from(chunk_po) >= r => AorClass::InAorStorer,
                // Known storer but this chunk is outside its radius,
                // or only ever seen forwarding: back of the queue.
                Some(_) => AorClass::Forwarder,
                None if a.storer_hits == 0 && a.forwarder_hits > 0 => AorClass::Forwarder,
                None => AorClass::Unknown,
            },
        }
    }
...
mod aor_tests {
    ...
    #[test]
    fn classifies_storer_forwarder_unknown() {
        let b = AorBook::default();
        let storer = PeerId::random();
        let fwd = PeerId::random();
        let fresh = PeerId::random();
        let so = [0xaa; 32];
        let fo = [0xbb; 32];
        b.record_receipt(storer, &so, &so, 8);
        b.record_receipt(fwd, &fo, &so, 8);
        assert_eq!(b.classify(&storer, 10), AorClass::InAorStorer);
        assert_eq!(b.classify(&storer, 5), AorClass::Forwarder);
        assert_eq!(b.classify(&fwd, 10), AorClass::Forwarder);
        assert_eq!(b.classify(&fresh, 10), AorClass::Unknown);
    }
}
